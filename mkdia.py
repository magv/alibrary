...
massive = "stTH"
opti = [f"--false=plink[{-1-2*i}]" for i, x in enumerate(fieldsi) if x not in massive] if len(fieldsi) > 1 else []
opto = [f"--false=plink[{-2-2*i}]" for i, x in enumerate(fieldso) if x not in massive] if len(fieldso) > 1 else []
